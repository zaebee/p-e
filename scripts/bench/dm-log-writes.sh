...
TS
  ...
  if [ "$variant" = "removed" ]; then
    ...
    sed -E 's|^([[:space:]]*)await syncPath\(inDir, "r", (options\.)?onSync\);$|\1// directory fsync removed for the control run|' \
  ...
  while "$REPLAY_LOG" --log "$LOG_LOOP" --replay "$REPLAY_LOOP" --next-flush >/dev/null 2>&1; do
    ...
    if mount -o ro "$REPLAY_LOOP" "$MNT" 2>/dev/null; then
      if [ -f "$MNT/relay/in/$name" ]; then
        ...
        local got
        got="$(sha256sum "$MNT/relay/in/$name" | cut -d' ' -f1)"
...
NAME="$(echo "$INFO" | sed 's/.*"name":"\([^"]*\)".*/\1/')"
DIGEST="$(echo "$INFO" | sed 's/.*"digest":"\([^"]*\)".*/\1/')"
echo "  published $NAME"
...
NAME2="$(echo "$INFO2" | sed 's/.*"name":"\([^"]*\)".*/\1/')"
DIGEST2="$(echo "$INFO2" | sed 's/.*"digest":"\([^"]*\)".*/\1/')"
