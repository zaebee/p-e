# p-e conformance report 08

A run of the falsifier over the frozen corpus.

## What changed since the previous run

The first run in which an invariant is falsified.
...
| I-3 | the observation is kept beside the conclusion | hivemark VIOLATES · apex UNDECIDABLE | **DEMOTED** |
...
- **hivemark — VIOLATES** *(OBSERVED)*. provenance.json pins 5 derivation inputs by digest and 5 of them are absent from the published corpus, so the producer publishes a conclusion whose input is not in the corpus — the falsifier's condition, stated
  - *projections: none. This finding rests on published bytes alone.*
