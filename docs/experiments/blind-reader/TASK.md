...
- `CATALOGUE.md` — nine invariants, each with a `falsifier:` clause and a
  `reader:` clause. This is the catalogue exactly as it stood **before any reader
  had run**, taken from the commit that froze it. Nothing has been added or
  removed for your benefit.
...
There exist, elsewhere: seven conformance reports with every verdict, an
implementation of a reader in TypeScript, fifty-five observations analysing the
findings, and a hundred-odd records of three agents arguing about them. None of it
is in this bundle. If you read any of it, this exercise produces nothing.
