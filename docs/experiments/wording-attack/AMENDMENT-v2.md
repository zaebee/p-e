# Amendment to issue-1 — digest domain and record identity, v2
...
v2 incorporates the repairs for SF-1, SF-3 and SF-4 from the Phase C implementation audit.
SF-2 was drafted, attacked and retracted; it is recorded separately as ADR-1 and Q8b
remains open.
...
> disagreement between the recorded and recomputed content identities changes none of
> them: the content is `PRESENT`, and its integrity verdict is a second, separate answer
> whose content this clause leaves open. **No store may report an integrity disagreement
> by moving a record out of `PRESENT`.**
>
...
## Amendment to MUST 5, second sentence

Replaces: *"Cross-authority references are **observations** and MUST be labelled as such."*

> A cross-authority reference travels as `ref:`, never as `parent:`. `parent:` asserts
> membership in the same chain and is scoped to one authority; `ref:` asserts an
> observation and carries the cross-store citation form `(store identity, locator,
> content digest)`. A `parent:` naming another authority is not a mislabelled observation
> — it is refused.
