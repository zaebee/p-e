...
Строка 18 лога — Settle, move в `history/YYYY-MM/` — `ABSENT`; `relay-0960`
объявил её закрываемой при D1 = A, никто не поднял. От привязки зависит статус
`expired/`:
...
- A. **Settle на публикации**: запись входит в `history/` при публикации,
  ...
  не нужно. Цена — неограниченная очередь.
...
4. Условие (2) D4 — перепроверку покрытия v8 — исполняет сторона, не
   писавшая ни v8, ни определение; атакует третья.
